...
def hydrostatics(myBodies, savepath='', CoB_override=None, rho=1025.0, g=9.81):
    '''
    ...
    for i, body in enumerate(myBodies):
        ...
        try:
            if capy_v2:
                ...
                body_hs = body.compute_hydrostatics(rho=rho)
                vo = body_hs['disp_volume']
                cb = np.array(CoB_override[i]) if CoB_override else body_hs['center_of_buoyancy']
                ...
                body_hs = mmhs.Hydrostatics(working_mesh=body_mesh, cog=body.center_of_mass, rho_water=rho, grav=g)
...
def call_capy(meshFName, wCapy, CoG=([0,0,0],), CoB=None, headings=[0.0], ncFName=None, 
              wDes=None, body_name=('',), depth=200.0, density=1025.0,
              lid_files=('',), additional_dofs_dir=None, num_threads=1):
    '''
    ...
    depth: float
        ...
        Capytaine outputting int32 types. Default is = np.inf
    density: float
    ...
    for i in np.arange(0, len(meshFName)):
        hull_mesh = cpt.io.mesh_loaders.load_mesh(meshFName[i])
        if i == 0:
            lid_mesh = cpt.io.mesh_loaders.load_mesh(lid_files[i]) 
        else:
            hull_mesh, lid_mesh = hull_mesh.extract_lid()
        body = cpt.FloatingBody(mesh=hull_mesh, ) # lid_mesh=lid_mesh
        body.center_of_mass = CoG[i]
        if body_name[i] != '':
            body.name = body_name[i]
    ...
    path += os.path.sep
    hydrostatics(bodies, path, rho=density)
...
def capy_solver(wCapy, CoG, headings, ncFName, wDes, body_name, depth, density,
    ...
    capyData = solver.fill_dataset(problems, [combo], hydrostatics=True)
